...
def main():
    ...
    try:
        if module.params['force'] or not os.path.exists(path):
            ...
            decrypted_content = Sops.decrypt(path, decode_output=False, output_type=get_data_type(module), rstrip=False, get_option_value=get_option_value)
        ...
        if changed and not module.check_mode:
            ...
            if path.endswith('.json'):
                output_type = 'json'
            data = Sops.encrypt(data=input_data, cwd=directory, input_type=input_type, output_type=output_type, get_option_value=get_option_value)
